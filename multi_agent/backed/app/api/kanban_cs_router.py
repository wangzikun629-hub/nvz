...
from multi_agent.backed.app.repositories import kanban_cs_repository, kanban_custom_column_repository
from multi_agent.backed.app.schemas.kanban_cs_schema import (
    CsRecordCreate,
    CsRecordUpdate,
    CsRecordResponse,
    CsRecordListResponse,
    CloseRequest,
...
router = APIRouter(prefix="/api/kanban/cs", tags=["客户服务看板"], dependencies=[Depends(_require_api_key)])
...
@router.get("/records", response_model=CsRecordListResponse)
def list_records(
    record_type: Optional[str] = Query(None, description="售前 / 售后"),
    is_closed: Optional[int] = Query(None, description="0=进行中, 1=已结题"),
    ...
    result = kanban_cs_repository.list_records(
...
@router.post("/records", response_model=CsRecordResponse, status_code=201)
def create_record(body: CsRecordCreate):
    data = body.model_dump(exclude_none=False)
    if data.get("start_date"):
        data["start_date"] = str(data["start_date"])
    if data.get("attachments"):
        data["attachments"] = [a.model_dump() if hasattr(a, "model_dump") else a for a in data["attachments"]]
    data["is_closed"] = int(data.get("is_closed", False))
    return kanban_cs_repository.create_record(data)


@router.get("/records/{record_id}", response_model=CsRecordResponse)
def get_record(record_id: int):
    rec = kanban_cs_repository.get_record(record_id)
    if not rec:
        raise HTTPException(status_code=404, detail="记录不存在")
    return rec


@router.put("/records/{record_id}", response_model=CsRecordResponse)
def update_record(record_id: int, body: CsRecordUpdate):
    ...
    if "is_closed" in data:
        data["is_closed"] = int(data["is_closed"])
    rec = kanban_cs_repository.update_record(record_id, data)
    if not rec:
        raise HTTPException(status_code=404, detail="记录不存在")
    return rec
...
def delete_record(record_id: int):
    ok = kanban_cs_repository.delete_record(record_id)
    if not ok:
        raise HTTPException(status_code=404, detail="记录不存在")
    return {"success": True}


@router.patch("/records/{record_id}/close")
def close_record(record_id: int, body: CloseRequest):
    rec = kanban_cs_repository.set_closed(record_id, body.is_closed)
    if not rec:
        raise HTTPException(status_code=404, detail="记录不存在")
    return {"success": True, "is_closed": rec["is_closed"]}


@router.get("/customers")
def get_customers():
    return {"customers": kanban_cs_repository.get_customers()}


@router.get("/owners")
def get_owners():
    return {"owners": kanban_cs_repository.get_owners()}


@router.get("/product_nos")
def get_product_nos():
    return {"product_nos": kanban_cs_repository.get_product_nos()}


@router.get("/case_types")
def get_case_types():
    return {"case_types": kanban_cs_repository.get_case_types()}


@router.get("/stats")
def get_stats():
    return kanban_cs_repository.get_stats()
...
def list_custom_columns():
    return {"columns": kanban_custom_column_repository.list_columns("cs")}


@router.post("/custom_columns", status_code=201)
def create_custom_column(body: Dict[str, Any]):
    try:
        col = kanban_custom_column_repository.create_column("cs", body.get("label", ""))
...
def rename_custom_column(field_key: str, body: Dict[str, Any]):
    try:
        col = kanban_custom_column_repository.rename_column("cs", field_key, body.get("label", ""))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    if not col:
...
def delete_custom_column(field_key: str):
    ok = kanban_custom_column_repository.delete_column("cs", field_key)
    if not ok:
        raise HTTPException(status_code=404, detail="列不存在")
    return {"success": True}
...
async def upload_plain_file(record_id: int, file: UploadFile = File(...)):
    ...
    entry = kanban_file_service.save_file("cs", record_id, filename, content)
    attachments = kanban_file_service.upsert_file_attachment(rec.get("attachments") or [], entry)
    kanban_cs_repository.update_record(record_id, {"attachments": attachments})
    return entry
...
async def download_plain_file(record_id: int, file_id: str):
    ...
    result = kanban_file_service.load_file("cs", record_id, file_id)
    if not result:
...
def delete_plain_file(record_id: int, file_id: str):
    ...
    if not rec:
        raise HTTPException(status_code=404, detail="记录不存在")
    kanban_file_service.delete_file("cs", record_id, file_id)
...
async def kb_upload(
    ...
    partition_id: str = Query(default="kanban_cs"),
):
...
async def parser_upload(
    ...
    """智能解析路线：上传 PDF/PPT/Word/Excel 到 KB parser 通道。"""
    rec = kanban_cs_repository.get_record(record_id)
    if not rec:
        raise HTTPException(status_code=404, detail="记录不存在")
    content  = await file.read()
...
async def parser_trigger(record_id: int, doc_id: str):
    ...
    attachments = kanban_kb_service.update_parser_attachment_by_doc_id(
        existing=rec.get("attachments") or [], doc_id=doc_id, parse_status="summarizing",
    )
    kanban_cs_repository.update_record(record_id, {"attachments": attachments})
...
async def parser_status(record_id: int, doc_id: str):
    ...
    if parse_status in terminal:
        attachments = kanban_kb_service.update_parser_attachment_by_doc_id(
            existing=rec.get("attachments") or [], doc_id=doc_id, parse_status=parse_status,
        )
        kanban_cs_repository.update_record(record_id, {"attachments": attachments})
...
async def parser_summary(record_id: int, doc_id: str):
    ...
    if not rec:
        raise HTTPException(status_code=404, detail="记录不存在")
    try:
        return await kanban_kb_service.get_parser_summary_by_doc(doc_id)
    except Exception as e:
...
async def parser_save_draft(record_id: int, summary_id: str, body: Dict[str, Any]):
    try:
        return await kanban_kb_service.save_parser_draft(summary_id, body.get("reviewed_json", {}))
    except Exception as e:
...
async def ai_query(body: KanbanAiQueryRequest):
    result = await kanban_ai_service.query(body.question, scope="cs")
    return result
